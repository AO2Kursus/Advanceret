/************************************************************************
 * @description: Modify from G33kDude's Brave.ahk v1
 * @author thqby
 * @date 2023/01/15
 * @version 1.0.1
 ***********************************************************************/
...
class Brave {
	...
	static FindInstance(exename := 'Brave.exe', debugport := 0) {
		items := Map()
		filter_items := Map()
	 ...
	 * @param URLs        - The page or array of pages for Brave to load when it opens
	 * @param Flags       - Additional flags for Brave when launching
	 * @param BravePath  - Path to Brave.exe, will detect from start menu when left blank
	 * @param DebugPort   - What port should Brave's remote debugging server run on
	*/
	__New(URLs := "about:blank", Flags := "", BravePath := "", DebugPort := 9222, ProfilePath := "") {
		; Verify BravePath
		try BravePath := RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Brave.exe")
		catch {
			throw Error('Brave could not be found')
		}
		; Verify DebugPort
		...
		SplitPath(BravePath, &exename)
		URLs := URLs is Array ? URLs : URLs && URLs is String ? [URLs] : []
		if instance := Brave.FindInstance(exename, DebugPort) {
		...
		Run(CliEscape(BravePath) ' --remote-debugging-port=' this.DebugPort
			(ProfilePath ? ' --user-data-dir=' CliEscape(ProfilePath) : '')
			(Flags ? ' ' Flags : '') URLString, , , &PID)
		if (hasother && Sleep(600) || !instance := Brave.FindInstance(exename, this.DebugPort))
			throw Error('Brave is not running in debug mode. Try closing all Brave processes and try again')
		this.PID := PID
	...
	/*
	 * End Brave by terminating the process.
	*/
	Kill() {
	...
	/*
	 * Queries Brave for a list of pages that expose a debug interface.
	...
	*/
	...
	NewTab(url := 'about:blank') {
		...
		try if ((PageData := JSON.parse(http.responseText)).Has('webSocketDebuggerUrl'))
				return Brave.Page(StrReplace(PageData['webSocketDebuggerUrl'], 'localhost', '127.0.0.1'), http)
	}
	...
	static GetPageBy(Key, Value, MatchMode := 'exact', Index := 1, fnCallback := '') {
		...
		for ii, PageData in this.GetPageList() {
			...
			if (((MatchMode = 'exact' && PageData[Key] = Value)	; Case insensitive
				...
				&& ++Count == Index)
				return Brave.Page(PageData['webSocketDebuggerUrl'], fnCallback)
		}
	}
	...
	class Page extends WebSocket {
		...
		Call(DomainAndMethod, Params?, WaitForResponse := true) {
			...
			if (response.Has('error'))
				throw Error('Brave indicated error in response', , JSON.stringify(response['error']))
		...
		Close() {
			...
			http := Brave._http, http.Open('GET', 'http://127.0.0.1:' m[1] '/json/close/' m[2]), http.Send()
			this.__Delete()
		}

		Activate() {
			http := Brave._http, RegExMatch(this.url, 'ws://[\d\.]+:(\d+)/devtools/page/(.+)$', &m)
		...
		WaitFor(JS, eq := ""){
			Loop {
				try{
					If(inStr(this.evaluate(JS)["value"], eq)){
						return this.evaluate(JS)["value"]
					}else{
						Sleep 100
						If(A_Index >= 50){
							MsgBox "Fandt ikke WaitFor(), reloading"
							reload
						}
					}
				}catch {
		...
		NavigateAndWait(destination, DesiredState:="complete", Interval:=500)
		{
			fromUrl := ""
			Loop{
				try
		...
		ValueArray(JS)
		{
			...
			;Eller JSON.stringify([].slice.call(document.querySelectorAll(JS)).map(function(e) { return e.innerText; }))
